...
def _extract_daily_arrays_from_db(args):
    """从数据库加载单个站点并提取每日电价数组，供并行调用。"""
    station_name, year = args
    try:
        df = load_price_data_from_db(station_name, year)
        if df is None or len(df.columns) < 2:
            return station_name, [], "无数据"

        date_col = df.columns[0]
        price_cols = df.columns[1:]
        if len(price_cols) < 96:
            return station_name, [], "列数不足"
        ...
        for _, row in df.iterrows():
            ...
            if len(prices) == 96 and not np.all(np.isnan(prices)):
                daily_arrays.append((date_str, prices))

        return station_name, daily_arrays, None
    except Exception as e:
        return station_name, [], str(e)


def _extract_daily_arrays_from_file(args):
    """从Excel文件加载单个站点并提取每日电价数组，供并行调用。"""
    file_path_str, modified_time_ns, file_size = args
    try:
        df = load_price_data(file_path_str, modified_time_ns, file_size)
        if df is None or len(df.columns) < 2:
            return file_path_str, [], "无数据"

        date_col_name = df.columns[0]
        price_cols = df.columns[1:]
        if len(price_cols) < 96:
            return file_path_str, [], "列数不足"
        price_cols = price_cols[:96]
        ...
        return file_path_str, daily_arrays, None
    except Exception as e:
...
def compute_provincial_average_from_db(station_year_pairs):
    ...
    # 并行加载和处理各站点数据
    max_workers = min(len(station_year_pairs), max(4, min(12, (os.cpu_count() or 4) + 2)))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = list(executor.map(_extract_daily_arrays_from_db, station_year_pairs))
    
    for station_name, daily_arrays, error in results:
        if error:
            failed_stations.append(station_name)
        else:
            all_daily_arrays.extend(daily_arrays)
...
def compute_provincial_average(file_keys):
    """计算全省所有站点各时段平均电价（并行加载），返回(date_list, time_columns, avg_by_date)"""
    all_daily_arrays = []

    # 并行加载和处理各站点数据
    max_workers = min(len(file_keys), max(4, min(12, (os.cpu_count() or 4) + 2)))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = list(executor.map(_extract_daily_arrays_from_file, file_keys))

    for _, daily_arrays, error in results:
        if not error:
            all_daily_arrays.extend(daily_arrays)
